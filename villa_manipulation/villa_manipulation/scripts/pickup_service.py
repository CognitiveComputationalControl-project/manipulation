...
from villa_manipulation.srv import (PickupObject, PickupObjectResponse)
...
def pickup(req):
	...
	res = PickupObjectResponse()

	try:
		obj_transform = get_transform(selected_object, 'map')
		# patt = robot.g_planner.getGraspWithObjectPose(req, robot.omni_base.pose, obj_transform)
		patt = robot.g_planner.get_grasp(req)

		obj_width = req.bbox_width
		if req.bbox_length < obj_width:
...
if __name__=='__main__':
	...
	s = rospy.Service('pickup', PickupObject, pickup)
	rospy.spin()
